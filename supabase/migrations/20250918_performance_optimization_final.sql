-- Performance Optimization for 63 Performance Advisor Warnings (FINAL FIXED VERSION)
-- This migration fixes RLS initialization and policy consolidation issues
-- Fixed: Removed references to non-existent columns (subscription_tier, is_completed)
...
-- FIXED: Only using columns that actually exist
...
-- FIXED: Using columns that actually exist
...
-- For goals queries (user + priority) - using actual column names
CREATE INDEX IF NOT EXISTS idx_goals_user_priority ON goals(user_id, priority)
WHERE user_id IS NOT NULL;

-- For goals queries (user + target amount for progress tracking)
CREATE INDEX IF NOT EXISTS idx_goals_user_target ON goals(user_id, target_amount)
WHERE user_id IS NOT NULL;
