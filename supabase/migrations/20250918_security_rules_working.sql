-- Working Security Rules Migration for PiggyBong
-- This version only applies security to tables that actually exist

-- First, create core tables that are referenced but may not exist
...
-- Create users table for app-specific user data (auth.users is managed by Supabase)
...
CREATE TABLE IF NOT EXISTS user_feedback (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
    type TEXT NOT NULL CHECK (type IN ('bug', 'feature', 'complaint', 'praise')),
...
-- Enable RLS on existing tables (skip auth.users - it's managed by Supabase)
...
ALTER TABLE IF EXISTS fan_idols ENABLE ROW LEVEL SECURITY;
...
-- FAN_IDOLS TABLE POLICIES (equivalent to user_artists)
CREATE POLICY "Users can only see own fan idols" ON fan_idols
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can only modify own fan idols" ON fan_idols
...
-- Grant table permissions conditionally
DO $$
BEGIN
    -- Grant on users table
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'users') THEN
        GRANT SELECT, INSERT, UPDATE, DELETE ON users TO authenticated;
    END IF;

    -- Grant on fan_idols table
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'fan_idols') THEN
        GRANT SELECT, INSERT, UPDATE, DELETE ON fan_idols TO authenticated;
    END IF;

    -- Grant on goals table
...
SELECT log_security_event('security_migration_completed', '{"version": "20250918_working"}'::jsonb);